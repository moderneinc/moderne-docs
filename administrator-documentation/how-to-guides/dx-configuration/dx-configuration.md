...
If you _do not_ plan on using Artifactory to store LST or recipe artifacts, please follow the [Maven repository configuration instructions](dx-configuration/configure-dx-with-maven-repository-access.md) and then jump to [Step 4](dx-configuration.md#step-4-optionally-configure-the-organizations-service).
...
If you want to set up this service, please see the [organizations service setup instructions](../references/architecture/organizations-service.md). Then, once it has been set up, please [configure the Moderne DX service to connect with the organization service](dx-configuration/configure-dx-org-service.md).
...
Some organizations want recipe artifacts to only come from locations configured in the Moderne DX service. If you want to configure that, please follow the [strict recipe sources instructions](dx-configuration/configure-dx-with-strict-recipe-sources.md).
...
If you have configured any services that require client SSL certificates (such as Maven or Artifactory), you will need to provide a KeyStore with these certificates. Please see the [configure DX with SSL certificate instructions](dx-configuration/configure-dx-ssl.md).
