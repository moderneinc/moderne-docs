...
const DocSidebarItemsWrapper: FunctionComponent<Props> = (props) => {
  ...
  const isPlatformPage =
    location.pathname.includes('/user-documentation/moderne-platform') ||
    location.pathname.includes('/administrator-documentation/moderne-platform');
  ...
  // For Platform pages at the top level, show toggle above the header
  const showPlatformToggle = isPlatformPage && !isNestedRendering && filteredItems.length > 0;

  return (
    <>
      {showPlatformToggle && (
        <li className={styles.platformToggleRow}>
          <PlatformToggle />
        </li>
      )}
