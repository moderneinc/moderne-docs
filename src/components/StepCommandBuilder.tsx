...
import ArtifactoryLSTSection from "./ArtifactoryLSTSection";
import ConfigStep from "./ConfigStep";
import MavenRepositorySection from "./MavenRepositorySection";
import SCMProviderSection from "./SCMProviderSection";
import StepCommandPreview from "./StepCommandPreview";
import { WizardStep, wizardSteps } from "./configSchema";
...
// Generic component props that all step components accept
...
const componentMap: Record<string, FC<ComponentProps>> = {
  ConfigStep,
  SCMStep: SCMProviderSection,
  ArtifactoryLSTStep: ArtifactoryLSTSection,
  MavenRepositoryStep: MavenRepositorySection,
...
const StepCommandBuilder: FC = () => {
  ...
  const currentStepDef: WizardStep = wizardSteps[currentStep];

  // Dynamically resolve the component for this step
  const StepComponent = currentStepDef.component
    ? componentMap[currentStepDef.component]
    : null;
  ...
  function loadFromLocalStorage(): FormValues | null {
    if (typeof window !== "undefined") {
      const savedState = localStorage.getItem(STORAGE_KEY);
      if (savedState) {
        const parsedState = JSON.parse(savedState) as SavedState;
  ...
  const goNext = (): void => {
    ...
    const isOptionalStepDisabled =
      isOptionalStep && !(formValues[currentStepKey] as any)?.enabled;

    // Either the step passes validation or it's a disabled optional step
    const isStepValid =
      !formState.errors[currentStepKey] || isOptionalStepDisabled;
    ...
    <FormProvider {...methods}>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        >
          ...
          >
            {wizardSteps.map((step, idx) => (
              <div
                ...
                } ${idx <= currentStep + 1 ? styles.clickable : ""}`}
          ...
          >
            <h3>
              Step {currentStep + 1}: {currentStepDef.label}
            </h3>
            <button
              type="button"
              onClick={resetForm}
...
export default StepCommandBuilder;
