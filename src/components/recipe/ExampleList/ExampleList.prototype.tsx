  ...
  <>
    {ex.name && (
      <div className={styles.exampleName}>
        <code className={styles.exampleTest}>{ex.name}</code>
      </div>
    )}
    {ex.parameters && ex.parameters.length > 0 && (
      <table className={styles.paramTable}>
        <thead>
          <tr>
            <th className={shared.th}>Parameter</th>
            <th className={shared.th}>Value</th>
          </tr>
        </thead>
        <tbody>
          {ex.parameters.map((p) => (
            <tr key={p.parameter}>
              ...
              <td className={shared.td}>
                <code className={shared.inlineCode}>{p.value}</code>
              </td>
            </tr>
          ))}
        </tbody>
...
/**
 * Examples as a collapsible accordion (see Accordion) — first one open. Replaces
 * the old "Show all N" reveal: many-example pages (counts run 0–26, median 1)
 * collapse into a tidy list instead of an ever-growing column, and every
 * example stays in the DOM for crawlers / the .md export.
 */
export const ExampleList: FunctionComponent<{ title: string; examples: RecipeExample[] }> = ({
  title,
  examples,
}) => {
