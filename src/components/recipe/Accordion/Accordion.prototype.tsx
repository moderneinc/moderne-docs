 ...
 * only when there's more than one) drives every row. Open state is controlled so
 * the toggle and individual clicks stay in sync.
 *
 * Every <details> keeps its body in the DOM when collapsed (CSS visibility, not
 * lazy-mount), so crawlers and the .md / llms.txt export read all of it.
 */
export const Accordion: FunctionComponent<{ title: ReactNode; items: AccordionItem[] }> = ({
  title,
  items,
}) => {
  ...
  return (
    <>
      {/* Section heading and the Expand/Collapse-all toggle share one row to save
          vertical space (the toggle sits at header level, not on its own line). */}
