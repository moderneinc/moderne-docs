...
<head>
  ...
  <link rel="canonical" href="https://docs.openrewrite.org/authoring-recipes/automate-breaking-changes" />
</head>
...
1. Rename the package `com.example.springboot2.starter` to `com.example.springboot3.starter`
  * This can be done with the [org.openrewrite.java.ChangePackage recipe](../../recipe-catalog/java/changepackage.md).
2. Change the type of `com.example.springboot2.starter.MyConfig` to `com.example.springboot3.starter.MyNewConfig`
  * This can be done with the [org.openrewrite.java.ChangeType recipe](../../recipe-catalog/java/changetype.md)
3. Upgrade the version of our library from `1.0.0` to `2.0.x` in the Gradle or Maven build file (e.g., `com.acme:inhouse-starter:1.0.0` --> `com.acme:inhouse-starter:2.0.x`)
  * This can be done with the [org.openrewrite.java.dependencies.UpgradeDependencyVersion recipe](../../recipe-catalog/java/dependencies/upgradedependencyversion.md)
...
Remember the best practice tip of [if it can be declarative, it should be declarative](../testing-and-best-practices/recipe-conventions-and-best-practices.md#if-it-can-be-declarative-it-should-be-declarative). For complex or custom recipes, this may not be possible, but for our example we can create a declarative recipe.

:::tip
If you need help with recipe creation, consider working through our [recipe authoring workshop](../../../../hands-on-learning/fundamentals/workshop-overview.md) or jumping back to the [writing a Java refactoring recipe guide](../writing-recipes/writing-a-java-refactoring-recipe.md).
:::
...
After creating a recipe, it's extremely important to test it so that we can ensure [we are doing no harm](../testing-and-best-practices/recipe-conventions-and-best-practices.md#do-no-harm). This is true even for declarative recipes.
