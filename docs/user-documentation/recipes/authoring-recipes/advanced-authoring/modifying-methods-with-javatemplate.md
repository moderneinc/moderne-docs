---
sidebar_label: Modifying methods with JavaTemplates
description: A deeper look into how you can use JavaTemplates to refactor/modify methods.
---

<head>
  <meta name="robots" content="noindex" />
  <link rel="canonical" href="https://docs.openrewrite.org/authoring-recipes/modifying-methods-with-javatemplate" />
</head>

# Modifying methods with JavaTemplates

The [Writing a Java refactoring recipe](../writing-recipes/writing-a-java-refactoring-recipe.md) guide walks through a recipe that adds a `hello()` method to a class if it doesn't already have one, using a `JavaTemplate` to create a basic method. However, a `JavaTemplate` can be used for much more complicated changes, such as refactoring existing methods. Let's explore that.
...
* You've already set up your [Recipe Development Environment](../set-up/recipe-development-environment.md)
* You've read through the [Java refactoring recipe guide](../writing-recipes/writing-a-java-refactoring-recipe.md) and the [recipe testing guide](../testing-and-best-practices/recipe-testing.md)
...
:::info
If you don't remember how to write tests or want more information about writing them, please see our [recipe testing guide](../testing-and-best-practices/recipe-testing.md)
:::
...
:::tip
If you need help deciding what [LST](../concepts/lossless-semantic-trees.md) to interact with (such as `MethodDeclaration` vs. `MethodInvocation`), check out our [Java LST examples guide](../concepts/lst-examples/java.md).
:::

### Remove the abstract modifier

Next up on our list of tasks is to remove the `abstract` modifier from our `setCustomerInfo()` method. [MethodDeclarations](../concepts/lst-examples/java.md#methoddeclaration) have a list of `Modifiers` in them. We can use a Java stream and the `filter` function to remove the `abstract` modifier:
...
As mentioned in our [best practice guide](../testing-and-best-practices/recipe-conventions-and-best-practices#avoid-constructing-lst-elements-by-hand), you should avoid constructing LST elements by hand. Instead, you should use the `JavaTemplate` class to construct any objects you need.
