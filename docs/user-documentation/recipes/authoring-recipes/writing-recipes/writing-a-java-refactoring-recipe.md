---
sidebar_label: Writing a Java refactoring recipe
...
<head>
  ...
  <link rel="canonical" href="https://docs.openrewrite.org/authoring-recipes/writing-a-java-refactoring-recipe" />
</head>
...
:::tip
We offer an entire [recipe authoring workshop](../../../../hands-on-learning/fundamentals/workshop-overview.md) that walks through everything you'd need to know to get started with recipes – from running them to the dev environment to writing different types of recipes. You may find it useful to work through that, too.
:::
...
This guide assumes you've already set up your [Recipe Development Environment](../set-up/recipe-development-environment.md) and that you are familiar with writing Java code.
...
:::info
For a more extensive look into recipe testing, check out the [Recipe Testing Guide](../testing-and-best-practices/recipe-testing.md)
:::
...
1. Figure out [which LSTs](../concepts/lossless-semantic-trees.md) have the data we need to make the changes we want.
...
For more information on visitors, please see our [visitors doc](../concepts/visitors.md). You may also find it beneficial to watch our video explaining visitors and how they work:
...
If we take a look at the [LST Examples doc](../concepts/lst-examples/java.md), we can see that a [J.ClassDeclaration](https://github.com/openrewrite/rewrite/blob/v7.34.0/rewrite-java/src/main/java/org/openrewrite/java/tree/J.java#L1062-L1336) has the information we need. It has a `FullyQualified` type that we can use to ensure we're only making changes on the specified class, and it contains a `Block` that includes `Statements` that may be `MethodDeclarations`, so we can check for a `hello()` method and potentially add one if it doesn't exist.
...
To filter out classes that already have a `hello()` method, we need to first figure out how to check for that. A good understanding of the [LSTs](../concepts/lst-examples/java.md) is necessary for this type of filtering. With that knowledge, we can build up a stream, filter it down, and check for any matches by doing something like:
...
In order to create complex LST elements such as a new method, it's a good idea to use a [Java Template](../concepts/javatemplate.md). At a high-level, Java Templates simplify the creation of LSTs by converting code snippets into fully created LSTs.
...
* [A deeper look into testing](../testing-and-best-practices/recipe-testing.md)
* [A best practice guide for making recipes](../testing-and-best-practices/recipe-conventions-and-best-practices.md)
* [A more complex recipe creation tutorial](../advanced-authoring/modifying-methods-with-javatemplate.md)
