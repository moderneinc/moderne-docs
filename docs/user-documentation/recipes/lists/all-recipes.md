---
description: A comprehensive list of all recipes organized by module.
---

# All Recipes by Module

_This doc indexes per-module recipe lists. Click a groupId to see its recipes._

Total recipes: 7279


## io.moderne.recipe

* [recipes-code-quality](all-recipes-io-moderne-recipe.md#recipes-code-quality)
* [recipes-kotlin](all-recipes-io-moderne-recipe.md#recipes-kotlin)
* [recipes-scala](all-recipes-io-moderne-recipe.md#recipes-scala)
* [rewrite-ai](all-recipes-io-moderne-recipe.md#rewrite-ai)
* [rewrite-angular](all-recipes-io-moderne-recipe.md#rewrite-angular)
* [rewrite-cryptography](all-recipes-io-moderne-recipe.md#rewrite-cryptography)
* [rewrite-cve-2026-22732](all-recipes-io-moderne-recipe.md#rewrite-cve-2026-22732)
* [rewrite-devcenter](all-recipes-io-moderne-recipe.md#rewrite-devcenter)
* [rewrite-dropwizard](all-recipes-io-moderne-recipe.md#rewrite-dropwizard)
* [rewrite-elastic](all-recipes-io-moderne-recipe.md#rewrite-elastic)
* [rewrite-hibernate](all-recipes-io-moderne-recipe.md#rewrite-hibernate)
* [rewrite-jasperreports](all-recipes-io-moderne-recipe.md#rewrite-jasperreports)
* [rewrite-java-application-server](all-recipes-io-moderne-recipe.md#rewrite-java-application-server)
* [rewrite-kafka](all-recipes-io-moderne-recipe.md#rewrite-kafka)
* [rewrite-nullability](all-recipes-io-moderne-recipe.md#rewrite-nullability)
* [rewrite-prethink](all-recipes-io-moderne-recipe.md#rewrite-prethink)
* [rewrite-program-analysis](all-recipes-io-moderne-recipe.md#rewrite-program-analysis)
* [rewrite-react](all-recipes-io-moderne-recipe.md#rewrite-react)
* [rewrite-release-metromap](all-recipes-io-moderne-recipe.md#rewrite-release-metromap)
* [rewrite-spring](all-recipes-io-moderne-recipe.md#rewrite-spring)
* [rewrite-tapestry](all-recipes-io-moderne-recipe.md#rewrite-tapestry)
* [rewrite-vulncheck](all-recipes-io-moderne-recipe.md#rewrite-vulncheck)

## org.openrewrite

* [rewrite-cobol](all-recipes-org-openrewrite.md#rewrite-cobol)
* [rewrite-core](all-recipes-org-openrewrite.md#rewrite-core)
* [rewrite-docker](all-recipes-org-openrewrite.md#rewrite-docker)
* [rewrite-gradle](all-recipes-org-openrewrite.md#rewrite-gradle)
* [rewrite-groovy](all-recipes-org-openrewrite.md#rewrite-groovy)
* [rewrite-hcl](all-recipes-org-openrewrite.md#rewrite-hcl)
* [rewrite-java](all-recipes-org-openrewrite.md#rewrite-java)
* [rewrite-javascript](all-recipes-org-openrewrite.md#rewrite-javascript)
* [rewrite-json](all-recipes-org-openrewrite.md#rewrite-json)
* [rewrite-kotlin](all-recipes-org-openrewrite.md#rewrite-kotlin)
* [rewrite-maven](all-recipes-org-openrewrite.md#rewrite-maven)
* [rewrite-properties](all-recipes-org-openrewrite.md#rewrite-properties)
* [rewrite-python](all-recipes-org-openrewrite.md#rewrite-python)
* [rewrite-toml](all-recipes-org-openrewrite.md#rewrite-toml)
* [rewrite-xml](all-recipes-org-openrewrite.md#rewrite-xml)
* [rewrite-yaml](all-recipes-org-openrewrite.md#rewrite-yaml)

## org.openrewrite.meta

* [rewrite-analysis](all-recipes-org-openrewrite-meta.md#rewrite-analysis)

## org.openrewrite.recipe

* [rewrite-all](all-recipes-org-openrewrite-recipe.md#rewrite-all)
* [rewrite-android](all-recipes-org-openrewrite-recipe.md#rewrite-android)
* [rewrite-apache](all-recipes-org-openrewrite-recipe.md#rewrite-apache)
* [rewrite-circleci](all-recipes-org-openrewrite-recipe.md#rewrite-circleci)
* [rewrite-codemods](all-recipes-org-openrewrite-recipe.md#rewrite-codemods)
* [rewrite-codemods-ng](all-recipes-org-openrewrite-recipe.md#rewrite-codemods-ng)
* [rewrite-compiled-analysis](all-recipes-org-openrewrite-recipe.md#rewrite-compiled-analysis)
* [rewrite-concourse](all-recipes-org-openrewrite-recipe.md#rewrite-concourse)
* [rewrite-cucumber-jvm](all-recipes-org-openrewrite-recipe.md#rewrite-cucumber-jvm)
* [rewrite-dotnet](all-recipes-org-openrewrite-recipe.md#rewrite-dotnet)
* [rewrite-feature-flags](all-recipes-org-openrewrite-recipe.md#rewrite-feature-flags)
* [rewrite-github-actions](all-recipes-org-openrewrite-recipe.md#rewrite-github-actions)
* [rewrite-gitlab](all-recipes-org-openrewrite-recipe.md#rewrite-gitlab)
* [rewrite-hibernate](all-recipes-org-openrewrite-recipe.md#rewrite-hibernate)
* [rewrite-jackson](all-recipes-org-openrewrite-recipe.md#rewrite-jackson)
* [rewrite-java-dependencies](all-recipes-org-openrewrite-recipe.md#rewrite-java-dependencies)
* [rewrite-java-security](all-recipes-org-openrewrite-recipe.md#rewrite-java-security)
* [rewrite-jenkins](all-recipes-org-openrewrite-recipe.md#rewrite-jenkins)
* [rewrite-joda](all-recipes-org-openrewrite-recipe.md#rewrite-joda)
* [rewrite-kubernetes](all-recipes-org-openrewrite-recipe.md#rewrite-kubernetes)
* [rewrite-liberty](all-recipes-org-openrewrite-recipe.md#rewrite-liberty)
* [rewrite-logging-frameworks](all-recipes-org-openrewrite-recipe.md#rewrite-logging-frameworks)
* [rewrite-micrometer](all-recipes-org-openrewrite-recipe.md#rewrite-micrometer)
* [rewrite-micronaut](all-recipes-org-openrewrite-recipe.md#rewrite-micronaut)
* [rewrite-migrate-java](all-recipes-org-openrewrite-recipe.md#rewrite-migrate-java)
* [rewrite-migrate-kotlin](all-recipes-org-openrewrite-recipe.md#rewrite-migrate-kotlin)
* [rewrite-migrate-python](all-recipes-org-openrewrite-recipe.md#rewrite-migrate-python)
* [rewrite-netty](all-recipes-org-openrewrite-recipe.md#rewrite-netty)
* [rewrite-nodejs](all-recipes-org-openrewrite-recipe.md#rewrite-nodejs)
* [rewrite-okhttp](all-recipes-org-openrewrite-recipe.md#rewrite-okhttp)
* [rewrite-openapi](all-recipes-org-openrewrite-recipe.md#rewrite-openapi)
* [rewrite-prethink](all-recipes-org-openrewrite-recipe.md#rewrite-prethink)
* [rewrite-quarkus](all-recipes-org-openrewrite-recipe.md#rewrite-quarkus)
* [rewrite-reactive-streams](all-recipes-org-openrewrite-recipe.md#rewrite-reactive-streams)
* [rewrite-rewrite](all-recipes-org-openrewrite-recipe.md#rewrite-rewrite)
* [rewrite-spring](all-recipes-org-openrewrite-recipe.md#rewrite-spring)
* [rewrite-spring-to-quarkus](all-recipes-org-openrewrite-recipe.md#rewrite-spring-to-quarkus)
* [rewrite-sql](all-recipes-org-openrewrite-recipe.md#rewrite-sql)
* [rewrite-static-analysis](all-recipes-org-openrewrite-recipe.md#rewrite-static-analysis)
* [rewrite-static-analysis-python](all-recipes-org-openrewrite-recipe.md#rewrite-static-analysis-python)
* [rewrite-struts](all-recipes-org-openrewrite-recipe.md#rewrite-struts)
* [rewrite-terraform](all-recipes-org-openrewrite-recipe.md#rewrite-terraform)
* [rewrite-testing-frameworks](all-recipes-org-openrewrite-recipe.md#rewrite-testing-frameworks)
* [rewrite-third-party](all-recipes-org-openrewrite-recipe.md#rewrite-third-party)
