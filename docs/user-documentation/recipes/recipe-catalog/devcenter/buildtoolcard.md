...
_Track build tool versions across repositories._

## Recipe source

[GitHub: BuildToolCard.java](https://github.com/moderneinc/rewrite-devcenter/blob/main/src/main/java/io/moderne/devcenter/BuildToolCard.java),
...
| --- | --- | --- | --- |
| `String` | cardName | The display name of the DevCenter card. | `Upgrade to Gradle 9` |
| `String` | buildTool | The build tool to track. Valid options: `Gradle`, `Maven`, `Bazel`, `ModerneCli` |  |
| `String` | targetVersion | The target version of the build tool. Specify the version out to the desired patch version. | `9.0.0` |
| `String` | fixRecipeId | *Optional*. The recipe to use to upgrade the build tool. | `org.openrewrite.gradle.MigrateToGradle9` |
...
This recipe has required configuration parameters. Recipes with required configuration parameters cannot be activated directly (unless you are running them via the Moderne CLI). To activate this recipe you must create a new recipe which fills in the required parameters. In your `rewrite.yml` create a new recipe with a unique name. For example: `com.yourorg.BuildToolCardExample`.
Here's how you can define and customize such a recipe within your rewrite.yml:
```yaml title="rewrite.yml"
---
type: specs.openrewrite.org/v1beta/recipe
name: com.yourorg.BuildToolCardExample
displayName: Build tool example
recipeList:
  - io.moderne.devcenter.BuildToolCard:
      cardName: Upgrade to Gradle 9
      buildTool: null
      targetVersion: 9.0.0
      fixRecipeId: org.openrewrite.gradle.MigrateToGradle9
```

Now that `com.yourorg.BuildToolCardExample` has been defined, activate it and take a dependency on `io.moderne.recipe:rewrite-devcenter:{{VERSION_IO_MODERNE_RECIPE_REWRITE_DEVCENTER}}` in your build file:
...
rewrite {
    activeRecipe("com.yourorg.BuildToolCardExample")
    ...
    <plugins>
      <plugin>
        ...
        <configuration>
          ...
          <activeRecipes>
            <recipe>com.yourorg.BuildToolCardExample</recipe>
          </activeRecipes>
        </configuration>
        <dependencies>
          <dependency>
...
<RecipeCallout link="https://app.moderne.io/recipes/io.moderne.devcenter.BuildToolCard" />
