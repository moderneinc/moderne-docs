...
<RecipeMeta
  ...
  fqName={"org.openrewrite.kotlin.migrate.ReplaceDeprecatedCapitalizeAndDecapitalize"}
  languages={["Kotlin"]}
...
<ExampleList examples={[{"variants":[{"language":"kotlin","before":"fun test(s: String) {\n    val c = s.capitalize()\n}\n","after":"fun test(s: String) {\n    val c = s.replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() }\n}\n","diff":"@@ -2,1 +2,1 @@\nfun test(s: String) {\n-   val c = s.capitalize()\n+   val c = s.replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() }\n}\n","newFile":false}]},{"variants":[{"language":"kotlin","before":"fun test(s: String) {\n    val c = s.capitalize()\n}\n","after":"fun test(s: String) {\n    val c = s.replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() }\n}\n","diff":"@@ -2,1 +2,1 @@\nfun test(s: String) {\n-   val c = s.capitalize()\n+   val c = s.replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() }\n}\n","newFile":false}]}]}>

## Examples

</ExampleList>

<UsageList usage={{"recipeName":"org.openrewrite.kotlin.migrate.ReplaceDeprecatedCapitalizeAndDecapitalize","displayName":"Replace deprecated `capitalize` and `decapitalize`","groupId":"org.openrewrite.recipe","artifactId":"rewrite-migrate-kotlin","versionKey":"VERSION_ORG_OPENREWRITE_RECIPE_REWRITE_MIGRATE_KOTLIN","requiresConfiguration":false}}>
