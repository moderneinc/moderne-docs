...
_Finds calls to `.get` on `Option` values. Calling `.get` on `Option` can throw `NoSuchElementException`; prefer `getOrElse`, `map`, `fold`, or pattern matching._
...
<RunRecipe
  recipeName="org.openrewrite.scala.recipes.cleanup.UseOptionSafely"
...
<RecipeCallout link="https://app.moderne.io/recipes/org.openrewrite.scala.recipes.cleanup.UseOptionSafely" />
