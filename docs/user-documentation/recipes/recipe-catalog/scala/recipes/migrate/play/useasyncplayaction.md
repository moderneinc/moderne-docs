...
_Finds Play controller actions (`Action {` or `Action.async {`) that contain `Await.result` blocking calls. Blocking inside Play Actions can exhaust the thread pool; use `Action.async` with `Future` instead._
...
<RunRecipe
  recipeName="org.openrewrite.scala.recipes.migrate.play.UseAsyncPlayAction"
...
<RecipeCallout link="https://app.moderne.io/recipes/org.openrewrite.scala.recipes.migrate.play.UseAsyncPlayAction" />
