...
_Replaces `.map(f).flatten` chains with `.flatMap(f)` for clarity and to avoid creating an unnecessary intermediate collection._
...
<RunRecipe
  recipeName="org.openrewrite.scala.recipes.performance.UseFlatMap"
...
<RecipeCallout link="https://app.moderne.io/recipes/org.openrewrite.scala.recipes.performance.UseFlatMap" />
