...
_Replaces `.filter(f).size` or `.filter(f).length` chains with `.count(f)` to avoid creating an unnecessary intermediate collection._
...
<RunRecipe
  recipeName="org.openrewrite.scala.recipes.performance.UseCount"
...
<RecipeCallout link="https://app.moderne.io/recipes/org.openrewrite.scala.recipes.performance.UseCount" />
