...
| `String` | block | Raw HCL of the nested block to insert, including the block type header and any labels. The block type (and labels) used for sibling lookup are derived from this value. | <pre>parameter {<br />  name  = "local_infile"<br />  value = "0"<br />}</pre> |
