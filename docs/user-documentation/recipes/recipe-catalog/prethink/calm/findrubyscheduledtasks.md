---
title: "Find Ruby scheduled tasks"
sidebar_label: "Find Ruby scheduled tasks"
hide_title: true
---
...
<RecipeMeta
  displayName={"Find Ruby scheduled tasks"}
  description={"Identify recurring background work in Ruby applications. Detects whenever schedules in `config/schedule.rb`, sidekiq-cron jobs created through `Sidekiq::Cron::Job.create` or declared under `:schedule:` in `config/sidekiq.yml`, clockwork `every` declarations in `clock.rb`, and rufus-scheduler `cron`/`every`/`in`/`at` calls."}
  fqName={"io.moderne.prethink.calm.FindRubyScheduledTasks"}
...
<RecipeHeader
  ...
  appLink={"https://app.moderne.io/recipes/io.moderne.prethink.calm.FindRubyScheduledTasks"}
  markdownUrl={"https://raw.githubusercontent.com/moderneinc/moderne-docs/refs/heads/main/docs/user-documentation/recipes/recipe-catalog/prethink/calm/findrubyscheduledtasks.md"}
  moderneOnly
>

<RecipeHeader.Title>Find Ruby scheduled tasks</RecipeHeader.Title>

<RecipeHeader.Description>Identify recurring background work in Ruby applications. Detects whenever schedules in `config/schedule.rb`, sidekiq-cron jobs created through `Sidekiq::Cron::Job.create` or declared under `:schedule:` in `config/sidekiq.yml`, clockwork `every` declarations in `clock.rb`, and rufus-scheduler `cron`/`every`/`in`/`at` calls.</RecipeHeader.Description>
