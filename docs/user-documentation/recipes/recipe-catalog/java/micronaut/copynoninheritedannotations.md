...
```shell title="shell"
mvn -U org.openrewrite.maven:rewrite-maven-plugin:run -Drewrite.recipeArtifactCoordinates=org.openrewrite.recipe:rewrite-micronaut:RELEASE -Drewrite.activeRecipes=org.openrewrite.java.micronaut.CopyNonInheritedAnnotations -Drewrite.exportDatatables=true
```
</TabItem>
...
```shell title="shell"
mod run . --recipe CopyNonInheritedAnnotations
```
