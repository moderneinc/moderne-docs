...
_In Jackson 3, `@JsonIgnore` on fields initialized with empty collections causes the field value to become `null` instead of maintaining the empty collection. This recipe replaces `@JsonIgnore` with `@JsonSetter(nulls = Nulls.AS_EMPTY)` on `Map` and `Collection` fields that have an empty collection initializer._
...
[GitHub: ReplaceJsonIgnoreWithJsonSetter.java](https://github.com/openrewrite/rewrite-jackson/blob/main/src/main/java/org/openrewrite/java/jackson/ReplaceJsonIgnoreWithJsonSetter.java),
...
<RunRecipe
  recipeName="org.openrewrite.java.jackson.ReplaceJsonIgnoreWithJsonSetter"
...
<RecipeCallout link="https://app.moderne.io/recipes/org.openrewrite.java.jackson.ReplaceJsonIgnoreWithJsonSetter" />
