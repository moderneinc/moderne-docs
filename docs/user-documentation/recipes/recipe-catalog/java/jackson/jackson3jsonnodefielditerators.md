...
_`JSONNode` fields are using `Collections` instead of `Iterator` singe Jackson 3. To mimic Jackson 2s behavior an additional call to `Collection#iterator()`is needed._
...
rewrite {
    activeRecipe("org.openrewrite.java.jackson.Jackson3JsonNodeFieldIterators")
    ...
    <plugins>
      <plugin>
        ...
        <configuration>
          ...
          <activeRecipes>
            <recipe>org.openrewrite.java.jackson.Jackson3JsonNodeFieldIterators</recipe>
          </activeRecipes>
        </configuration>
        <dependencies>
          <dependency>
...
```shell title="shell"
mvn -U org.openrewrite.maven:rewrite-maven-plugin:run -Drewrite.recipeArtifactCoordinates=org.openrewrite.recipe:rewrite-jackson:RELEASE -Drewrite.activeRecipes=org.openrewrite.java.jackson.Jackson3JsonNodeFieldIterators -Drewrite.exportDatatables=true
```
</TabItem>
...
```shell title="shell"
mod run . --recipe Jackson3JsonNodeFieldIterators
```
...
<RecipeCallout link="https://app.moderne.io/recipes/org.openrewrite.java.jackson.Jackson3JsonNodeFieldIterators" />
