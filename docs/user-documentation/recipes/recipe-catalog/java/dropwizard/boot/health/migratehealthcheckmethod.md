...
_Transforms `Result.healthy()`/`Result.unhealthy()` calls to `Health.up().build()`/`Health.down().build()` and wraps throwing `check()` methods in try-catch._
...
<RunRecipe
  recipeName="io.moderne.java.dropwizard.boot.health.MigrateHealthCheckMethod"
...
<RecipeCallout link="https://app.moderne.io/recipes/io.moderne.java.dropwizard.boot.health.MigrateHealthCheckMethod" />
