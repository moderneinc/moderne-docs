...
_Adds `@PostConstruct` to `start()` and `@PreDestroy` to `stop()` on classes implementing `io.dropwizard.lifecycle.Managed`._
...
<RunRecipe
  recipeName="io.moderne.java.dropwizard.boot.lifecycle.MigrateManagedLifecycle"
...
<RecipeCallout link="https://app.moderne.io/recipes/io.moderne.java.dropwizard.boot.lifecycle.MigrateManagedLifecycle" />
