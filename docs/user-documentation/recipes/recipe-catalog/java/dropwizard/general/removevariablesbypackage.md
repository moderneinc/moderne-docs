...
<head>
  <link rel="canonical" href="https://docs.openrewrite.org/recipes/java/dropwizard/general/removevariablesbypackage" />
</head>
...
_Removes class-level variables from classes in the specified package._
...
| `Boolean` | removeOnlyClassScope | Ignores variables that are method scope | `com.example.unwanted` |
...
This recipe has required configuration parameters. Recipes with required configuration parameters cannot be activated directly (unless you are running them via the Moderne CLI). To activate this recipe you must create a new recipe which fills in the required parameters. In your `rewrite.yml` create a new recipe with a unique name. For example: `com.yourorg.RemoveVariablesByPackageExample`.
...
recipeList:
  - org.openrewrite.java.dropwizard.general.RemoveVariablesByPackage:
      ...
      removeOnlyClassScope: com.example.unwanted
```

Now that `com.yourorg.RemoveVariablesByPackageExample` has been defined, activate it and take a dependency on `org.openrewrite.recipe:rewrite-dropwizard:{{VERSION_ORG_OPENREWRITE_RECIPE_REWRITE_DROPWIZARD}}` in your build file:
...
rewrite {
    activeRecipe("com.yourorg.RemoveVariablesByPackageExample")
    ...
    <plugins>
      <plugin>
        ...
        <configuration>
          ...
          <activeRecipes>
            <recipe>com.yourorg.RemoveVariablesByPackageExample</recipe>
          </activeRecipes>
        </configuration>
        <dependencies>
          <dependency>
...
```shell title="shell"
mod run . --recipe RemoveVariablesByPackage --recipe-option "packageFilter=com.example.unwanted" --recipe-option "removeOnlyClassScope=com.example.unwanted"
```
...
<RecipeCallout link="https://app.moderne.io/recipes/org.openrewrite.java.dropwizard.general.RemoveVariablesByPackage" />
