...
_Configures the Gradle `jar` task to produce a self-contained fat JAR with all runtime dependencies bundled. Supports both Groovy DSL (`build.gradle`) and Kotlin DSL (`build.gradle.kts`)._
...
name: com.yourorg.ConfigureGradleFatJarExample
displayName: Configure Gradle `jar` task for fat JAR example
recipeList:
  - io.moderne.java.server.jboss.ConfigureGradleFatJar:
      mainClass: com.example.JettyServer
```
...
<RunRecipe
  recipeName="io.moderne.java.server.jboss.ConfigureGradleFatJar"
...
<RecipeCallout link="https://app.moderne.io/recipes/io.moderne.java.server.jboss.ConfigureGradleFatJar" />
