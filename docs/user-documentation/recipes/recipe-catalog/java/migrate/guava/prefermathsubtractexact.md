...
```shell title="shell"
mvn -U org.openrewrite.maven:rewrite-maven-plugin:run -Drewrite.recipeArtifactCoordinates=org.openrewrite.recipe:rewrite-migrate-java:RELEASE -Drewrite.activeRecipes=org.openrewrite.java.migrate.guava.PreferMathSubtractExact -Drewrite.exportDatatables=true
```
</TabItem>
...
```shell title="shell"
mod run . --recipe PreferMathSubtractExact
```
