...
This recipe has required configuration parameters. Recipes with required configuration parameters cannot be activated directly (unless you are running them via the Moderne CLI). To activate this recipe you must create a new recipe which fills in the required parameters. In your `rewrite.yml` create a new recipe with a unique name. For example: `com.yourorg.ResultOfMethodCallIgnoredExample`.
...
```shell title="shell"
mod run . --recipe ResultOfMethodCallIgnored --recipe-option "methodPattern=java.io.File mkdir*()"
```
...
<RecipeCallout link="https://app.moderne.io/recipes/org.openrewrite.java.search.ResultOfMethodCallIgnored" />
