...
_Finds uses of Spring Security password encoders that are unsuitable for production password storage: `NoOpPasswordEncoder` (plaintext), `StandardPasswordEncoder` (deprecated SHA-256), `MessageDigestPasswordEncoder` (raw message digest), `Md4PasswordEncoder` (MD4, broken), and `LdapShaPasswordEncoder` (deprecated). Use an adaptive function such as `BCryptPasswordEncoder`, `Argon2PasswordEncoder`, `Pbkdf2PasswordEncoder`, or `SCryptPasswordEncoder` instead._
...
<RunRecipe
  recipeName="org.openrewrite.java.security.search.FindWeakSpringPasswordEncoder"
...
<RecipeCallout link="https://app.moderne.io/recipes/org.openrewrite.java.security.search.FindWeakSpringPasswordEncoder" />
