...
_Replace `MockitoTestExecutionListener` in projects that have TestNG as a dependency. Uses `MockitoAnnotations.openMocks(this)` with `@BeforeMethod`/`@AfterMethod` as the replacement._
...
<RunRecipe
  recipeName="org.openrewrite.java.testing.mockito.ReplaceMockitoTestExecutionListenerForTestNG"
  displayName="Replace `MockitoTestExecutionListener` (TestNG projects)"
...
<RecipeCallout link="https://app.moderne.io/recipes/org.openrewrite.java.testing.mockito.ReplaceMockitoTestExecutionListenerForTestNG" />
