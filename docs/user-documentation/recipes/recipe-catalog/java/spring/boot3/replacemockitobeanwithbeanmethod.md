...
_`@MockitoBean` and `@MockitoSpyBean` only work in test classes, not in `@Configuration` classes. This recipe converts annotated fields into `@Bean` methods using `Mockito.mock()` or `Mockito.spy()`._
...
<RunRecipe
  recipeName="io.moderne.java.spring.boot3.ReplaceMockitoBeanWithBeanMethod"
...
<RecipeCallout link="https://app.moderne.io/recipes/io.moderne.java.spring.boot3.ReplaceMockitoBeanWithBeanMethod" />
