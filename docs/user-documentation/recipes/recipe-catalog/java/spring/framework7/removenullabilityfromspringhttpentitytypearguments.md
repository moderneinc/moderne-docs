...
_Spring Framework 7 narrowed `HttpEntity<T>` (and its subtypes `ResponseEntity` and `RequestEntity`) to `<T : Any>`. This recipe removes Kotlin's `?` nullable marker from the type argument of these types in declared parameterized types and in explicit method-invocation type arguments, so Kotlin sources continue to compile._
...
<RunRecipe
  recipeName="io.moderne.java.spring.framework7.RemoveNullabilityFromSpringHttpEntityTypeArguments"
...
<RecipeCallout link="https://app.moderne.io/recipes/io.moderne.java.spring.framework7.RemoveNullabilityFromSpringHttpEntityTypeArguments" />
