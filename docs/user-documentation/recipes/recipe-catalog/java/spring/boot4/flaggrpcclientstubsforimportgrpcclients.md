---
title: "Flag gRPC client stub injections that need `@ImportGrpcClients`"
sidebar_label: "Flag gRPC client stub injections that need `@ImportGrpcClients`"
hide_title: true
---
...
<RecipeMeta
  displayName={"Flag gRPC client stub injections that need `@ImportGrpcClients`"}
  description={"Spring gRPC 1.0 auto-scanned generated proto stubs and registered them as client beans; Spring gRPC 1.1 no longer does, so each stub must be imported explicitly with `@ImportGrpcClients`. The correct stub list and annotation location are application-specific, so rather than editing code this recipe adds a TODO comment to classes that inject a gRPC client stub (a field or constructor/method parameter whose type extends `io.grpc.stub.AbstractStub`) so the annotation can be added by hand."}
  fqName={"io.moderne.java.spring.boot4.FlagGrpcClientStubsForImportGrpcClients"}
...
<RecipeHeader
  ...
  appLink={"https://app.moderne.io/recipes/io.moderne.java.spring.boot4.FlagGrpcClientStubsForImportGrpcClients"}
  markdownUrl={"https://raw.githubusercontent.com/moderneinc/moderne-docs/refs/heads/main/docs/user-documentation/recipes/recipe-catalog/java/spring/boot4/flaggrpcclientstubsforimportgrpcclients.md"}
  moderneOnly
>

<RecipeHeader.Title>Flag gRPC client stub injections that need `@ImportGrpcClients`</RecipeHeader.Title>

<RecipeHeader.Description>Spring gRPC 1.0 auto-scanned generated proto stubs and registered them as client beans; Spring gRPC 1.1 no longer does, so each stub must be imported explicitly with `@ImportGrpcClients`. The correct stub list and annotation location are application-specific, so rather than editing code this recipe adds a TODO comment to classes that inject a gRPC client stub (a field or constructor/method parameter whose type extends `io.grpc.stub.AbstractStub`) so the annotation can be added by hand.</RecipeHeader.Description>
