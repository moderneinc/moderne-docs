...
<RecipeHeader.Title>Replace deprecated `Buffer.slice()` with `Buffer.subarray()`</RecipeHeader.Title>

<RecipeHeader.Description>Replace deprecated `buffer.slice()` calls with `buffer.subarray()` for compatibility with Uint8Array.prototype.slice().</RecipeHeader.Description>
