...
<RecipeMeta
  ...
  description={"Raises the lower bound of the `engines.node` version range in package.json to the specified Node.js version, performing a hard cutover that drops support for older, end-of-life versions (`22.x` → `24.x`, `>= 22` → `>= 24`). The original constraint style is preserved where possible and the version is never lowered."}
...
<RecipeHeader.Title>Increase Node.js engine version</RecipeHeader.Title>

<RecipeHeader.Description>Raises the lower bound of the `engines.node` version range in package.json to the specified Node.js version, performing a hard cutover that drops support for older, end-of-life versions (`22.x` → `24.x`, `>= 22` → `>= 24`). The original constraint style is preserved where possible and the version is never lowered.</RecipeHeader.Description>
