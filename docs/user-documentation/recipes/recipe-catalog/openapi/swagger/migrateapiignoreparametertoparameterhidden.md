...
_Springfox's `@ApiIgnore` is commonly placed on framework-injected controller parameters (`Principal`, `HttpServletRequest`, `Pageable`, ...). A flat `ChangeType` to `io.swagger.v3.oas.annotations.Hidden` produces code that does not compile, because `@Hidden` cannot target parameters. Convert parameter usages directly to `@io.swagger.v3.oas.annotations.Parameter(hidden = true)` and leave method/class-level `@ApiIgnore` for the subsequent `ChangeType` step._

## Recipe source

[GitHub: MigrateApiIgnoreParameterToParameterHidden.java](https://github.com/openrewrite/rewrite-openapi/blob/main/src/main/java/org/openrewrite/openapi/swagger/MigrateApiIgnoreParameterToParameterHidden.java),
...
<RecipeCallout link="https://app.moderne.io/recipes/org.openrewrite.openapi.swagger.MigrateApiIgnoreParameterToParameterHidden" />
