---
title: "Add `@MonotonicNonNull` to an uninitialized field"
sidebar_label: "Add `@MonotonicNonNull` to an uninitialized field"
hide_title: true
---
...
<RecipeHeader
  ...
  appLink={"https://app.moderne.io/recipes/io.moderne.nullability.AddMonotonicNonNullToUninitializedField"}
  markdownUrl={"https://raw.githubusercontent.com/moderneinc/moderne-docs/refs/heads/main/docs/user-documentation/recipes/recipe-catalog/nullability/addmonotonicnonnulltouninitializedfield.md"}
  moderneOnly
>

<RecipeHeader.Title>Add `@MonotonicNonNull` to an uninitialized field</RecipeHeader.Title>

<RecipeHeader.Description>Adds the Checker Framework `@MonotonicNonNull` to a non-primitive, non-`final` reference field inside a `@NullMarked` scope that has no nullability annotation, no initializer, no dependency-injection annotation, and is not definitely assigned by the end of construction (or is read before assignment) — the condition for NullAway's "@NonNull field not initialized" error. A field that is also assigned a literal `null` is genuinely nullable and gets JSpecify `@Nullable` instead. Java sources only; idempotent; only annotations are added.</RecipeHeader.Description>
