...
**OpenRewrite.CSharp.Recipes.RemoveDotNetCliToolReference**

_Removes a `<DotNetCliToolReference>` element from .csproj files. Use `*` to remove every CLI tool reference._
...
| --- | --- | --- | --- |
| `String` | ToolName | The CLI tool package name to remove. Supports glob patterns. Use `*` to remove all CLI tool references. | `Microsoft.DotNet.Watcher.Tools` |
