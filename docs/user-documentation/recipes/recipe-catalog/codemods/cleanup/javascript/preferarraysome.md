...
<head>
  <link rel="canonical" href="https://docs.openrewrite.org/recipes/codemods/cleanup/javascript/preferarraysome" />
</head>
...
<RecipeMeta
  ...
  fqName={"org.openrewrite.codemods.cleanup.javascript.PreferArraySome"}
...
<RecipeHeader
  ...
  appLink={"https://app.moderne.io/recipes/org.openrewrite.codemods.cleanup.javascript.PreferArraySome"}
  markdownUrl={"https://raw.githubusercontent.com/moderneinc/moderne-docs/refs/heads/main/docs/user-documentation/recipes/recipe-catalog/codemods/cleanup/javascript/preferarraysome.md"}
>

<RecipeHeader.Title>Prefer `.some()` over `.filter().length` check and `.{find,findLast}()`</RecipeHeader.Title>

<RecipeHeader.Description>Prefer `.some()` over `.filter().length` check and `.{find,findLast}()`. See [rule details](https://github.com/sindresorhus/eslint-plugin-unicorn/blob/main/docs/rules/prefer-array-some.md).</RecipeHeader.Description>
...
<UsageList usage={{"recipeName":"org.openrewrite.codemods.cleanup.javascript.PreferArraySome","displayName":"Prefer `.some()` over `.filter().length` check and `.{find,findLast}()`","groupId":"org.openrewrite.recipe","artifactId":"rewrite-codemods","versionKey":"VERSION_ORG_OPENREWRITE_RECIPE_REWRITE_CODEMODS","requiresConfiguration":false}}>
