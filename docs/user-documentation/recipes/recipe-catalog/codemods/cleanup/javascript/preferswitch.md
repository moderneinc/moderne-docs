...
_Prefer `switch` over multiple `else-if`. See [rule details](https://github.com/sindresorhus/eslint-plugin-unicorn/blob/main/docs/rules/prefer-switch.md)._
...
* [Lint source code with ESLint](../../../codemods/eslint)
  * fix: `true`
  * configFile: `{   "root": true,   "parser": "@typescript-eslint/parser",   "parserOptions": { "project": ["./tsconfig.json"] },   "plugins": ["unicorn"],   "rules": {     "unicorn/prefer-switch": 2   },   "globals": {     "browser": true,     "node": true   } } `
...
rewrite {
    activeRecipe("org.openrewrite.codemods.cleanup.javascript.PreferSwitch")
    ...
    <plugins>
      <plugin>
        ...
        <configuration>
          ...
          <activeRecipes>
            <recipe>org.openrewrite.codemods.cleanup.javascript.PreferSwitch</recipe>
          </activeRecipes>
        </configuration>
        <dependencies>
          <dependency>
...
```shell title="shell"
mvn -U org.openrewrite.maven:rewrite-maven-plugin:run -Drewrite.recipeArtifactCoordinates=org.openrewrite.recipe:rewrite-codemods:RELEASE -Drewrite.activeRecipes=org.openrewrite.codemods.cleanup.javascript.PreferSwitch -Drewrite.exportDatatables=true
```
</TabItem>
...
<RecipeCallout link="https://app.moderne.io/recipes/org.openrewrite.codemods.cleanup.javascript.PreferSwitch" />
