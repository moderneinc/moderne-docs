...
<RecipeMeta
  displayName={"Prefer `.find()` and `.findLast()` over the first or last element from `.filter()`"}
  ...
  fqName={"org.openrewrite.codemods.cleanup.javascript.PreferArrayFind"}
...
<RecipeHeader
  ...
  appLink={"https://app.moderne.io/recipes/org.openrewrite.codemods.cleanup.javascript.PreferArrayFind"}
  markdownUrl={"https://raw.githubusercontent.com/moderneinc/moderne-docs/refs/heads/main/docs/user-documentation/recipes/recipe-catalog/codemods/cleanup/javascript/preferarrayfind.md"}
>

<RecipeHeader.Title>Prefer `.find()` and `.findLast()` over the first or last element from `.filter()`</RecipeHeader.Title>

<RecipeHeader.Description>Prefer `.find()` and `.findLast()` over the first or last element from `.filter()`. See [rule details](https://github.com/sindresorhus/eslint-plugin-unicorn/blob/main/docs/rules/prefer-array-find.md).</RecipeHeader.Description>
...
<UsageList usage={{"recipeName":"org.openrewrite.codemods.cleanup.javascript.PreferArrayFind","displayName":"Prefer `.find()` and `.findLast()` over the first or last element from `.filter()`","groupId":"org.openrewrite.recipe","artifactId":"rewrite-codemods","versionKey":"VERSION_ORG_OPENREWRITE_RECIPE_REWRITE_CODEMODS","requiresConfiguration":false}}>
