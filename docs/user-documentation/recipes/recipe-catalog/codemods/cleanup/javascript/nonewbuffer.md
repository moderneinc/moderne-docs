...
<head>
  <link rel="canonical" href="https://docs.openrewrite.org/recipes/codemods/cleanup/javascript/nonewbuffer" />
</head>

import { RecipeHeader, RecipeMeta, RecipeList, OptionsTable, ExampleList, UsageList, DataTableList } from '@site/src/components/recipe';

<RecipeMeta
  displayName={"Enforce the use of `Buffer.from()` and `Buffer.alloc()` instead of the deprecated `new Buffer()`"}
  ...
  fqName={"org.openrewrite.codemods.cleanup.javascript.NoNewBuffer"}
...
<RecipeHeader
  ...
  appLink={"https://app.moderne.io/recipes/org.openrewrite.codemods.cleanup.javascript.NoNewBuffer"}
  markdownUrl={"https://raw.githubusercontent.com/moderneinc/moderne-docs/refs/heads/main/docs/user-documentation/recipes/recipe-catalog/codemods/cleanup/javascript/nonewbuffer.md"}
>

<RecipeHeader.Title>Enforce the use of `Buffer.from()` and `Buffer.alloc()` instead of the deprecated `new Buffer()`</RecipeHeader.Title>

<RecipeHeader.Description>Enforce the use of `Buffer.from()` and `Buffer.alloc()` instead of the deprecated `new Buffer()`. See [rule details](https://github.com/sindresorhus/eslint-plugin-unicorn/blob/main/docs/rules/no-new-buffer.md).</RecipeHeader.Description>
...
<UsageList usage={{"recipeName":"org.openrewrite.codemods.cleanup.javascript.NoNewBuffer","displayName":"Enforce the use of `Buffer.from()` and `Buffer.alloc()` instead of the deprecated `new Buffer()`","groupId":"org.openrewrite.recipe","artifactId":"rewrite-codemods","versionKey":"VERSION_ORG_OPENREWRITE_RECIPE_REWRITE_CODEMODS","requiresConfiguration":false}}>
