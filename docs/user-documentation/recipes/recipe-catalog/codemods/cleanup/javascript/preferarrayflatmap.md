...
<head>
  <link rel="canonical" href="https://docs.openrewrite.org/recipes/codemods/cleanup/javascript/preferarrayflatmap" />
</head>
...
<RecipeMeta
  ...
  fqName={"org.openrewrite.codemods.cleanup.javascript.PreferArrayFlatMap"}
...
<RecipeHeader
  ...
  appLink={"https://app.moderne.io/recipes/org.openrewrite.codemods.cleanup.javascript.PreferArrayFlatMap"}
  markdownUrl={"https://raw.githubusercontent.com/moderneinc/moderne-docs/refs/heads/main/docs/user-documentation/recipes/recipe-catalog/codemods/cleanup/javascript/preferarrayflatmap.md"}
>

<RecipeHeader.Title>Prefer `.flatMap()` over `.map().flat()`</RecipeHeader.Title>

<RecipeHeader.Description>Prefer `.flatMap()` over `.map().flat()`. See [rule details](https://github.com/sindresorhus/eslint-plugin-unicorn/blob/main/docs/rules/prefer-array-flat-map.md).</RecipeHeader.Description>
...
<UsageList usage={{"recipeName":"org.openrewrite.codemods.cleanup.javascript.PreferArrayFlatMap","displayName":"Prefer `.flatMap()` over `.map().flat()`","groupId":"org.openrewrite.recipe","artifactId":"rewrite-codemods","versionKey":"VERSION_ORG_OPENREWRITE_RECIPE_REWRITE_CODEMODS","requiresConfiguration":false}}>
