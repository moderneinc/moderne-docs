...
<ExampleList examples={[{"parameters":[{"parameter":"markSource","value":"null"}],"variants":[{"language":"java","before":"class Test {\n    String sql = \"select * from users where name like '%smith'\";\n}\n","after":"class Test {\n    String sql = /*~~(select-star: `SELECT *` fetches every column; enumerate only the columns the code uses)~~>*//*~~(leading-wildcard-like: `LIKE '%smith'` starts with a wildcard, so no index prefix can narrow the match and every row is examined; a trigram or full-text index supports this kind of search)~~>*/\"select * from users where name like '%smith'\";\n}\n","diff":"@@ -2,1 +2,1 @@\nclass Test {\n-   String sql = \"select * from users where name like '%smith'\";\n+   String sql = /*~~(select-star: `SELECT *` fetches every column; enumerate only the columns the code uses)~~>*//*~~(leading-wildcard-like: `LIKE '%smith'` starts with a wildcard, so no index prefix can narrow the match and every row is examined; a trigram or full-text index supports this kind of search)~~>*/\"select * from users where name like '%smith'\";\n}\n","newFile":false}]}]}>
