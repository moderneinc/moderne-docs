---
title: "Migrate `Cluster` to `cluster.x-k8s.io/v1beta2`"
sidebar_label: "Migrate `Cluster` to `cluster.x-k8s.io/v1beta2`"
hide_title: true
---
...
<RecipeMeta
  displayName={"Migrate `Cluster` to `cluster.x-k8s.io/v1beta2`"}
  description={"Rewrite a `Cluster` into its `v1beta2` form. `spec.topology.class` becomes `spec.topology.classRef.name`, and a `spec.topology.rolloutAfter` is reported rather than dropped."}
  fqName={"org.openrewrite.kubernetes.clusterapi.MigrateClusterToV1beta2"}
...
<RecipeHeader
  ...
  appLink={"https://app.moderne.io/recipes/org.openrewrite.kubernetes.clusterapi.MigrateClusterToV1beta2"}
  markdownUrl={"https://raw.githubusercontent.com/moderneinc/moderne-docs/refs/heads/main/docs/user-documentation/recipes/recipe-catalog/kubernetes/clusterapi/migrateclustertov1beta2.md"}
  moderneOnly
>

<RecipeHeader.Title>Migrate `Cluster` to `cluster.x-k8s.io/v1beta2`</RecipeHeader.Title>

<RecipeHeader.Description>Rewrite a `Cluster` into its `v1beta2` form. `spec.topology.class` becomes `spec.topology.classRef.name`, and a `spec.topology.rolloutAfter` is reported rather than dropped.</RecipeHeader.Description>
...
<UsageList usage={{"recipeName":"org.openrewrite.kubernetes.clusterapi.MigrateClusterToV1beta2","displayName":"Migrate `Cluster` to `cluster.x-k8s.io/v1beta2`","groupId":"org.openrewrite.recipe","artifactId":"rewrite-kubernetes","versionKey":"VERSION_ORG_OPENREWRITE_RECIPE_REWRITE_KUBERNETES","requiresConfiguration":false}}>
