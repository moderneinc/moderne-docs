---
title: "Migrate `MachinePool` to `cluster.x-k8s.io/v1beta2`"
sidebar_label: "Migrate `MachinePool` to `cluster.x-k8s.io/v1beta2`"
hide_title: true
---
...
<RecipeMeta
  displayName={"Migrate `MachinePool` to `cluster.x-k8s.io/v1beta2`"}
  description={"Rewrite a `MachinePool` into its `v1beta2` form. `spec.minReadySeconds` moves onto the machines it creates, since `MachinePoolSpec` no longer holds it."}
  fqName={"org.openrewrite.kubernetes.clusterapi.MigrateMachinePoolToV1beta2"}
...
<RecipeHeader
  ...
  appLink={"https://app.moderne.io/recipes/org.openrewrite.kubernetes.clusterapi.MigrateMachinePoolToV1beta2"}
  markdownUrl={"https://raw.githubusercontent.com/moderneinc/moderne-docs/refs/heads/main/docs/user-documentation/recipes/recipe-catalog/kubernetes/clusterapi/migratemachinepooltov1beta2.md"}
  moderneOnly
>

<RecipeHeader.Title>Migrate `MachinePool` to `cluster.x-k8s.io/v1beta2`</RecipeHeader.Title>

<RecipeHeader.Description>Rewrite a `MachinePool` into its `v1beta2` form. `spec.minReadySeconds` moves onto the machines it creates, since `MachinePoolSpec` no longer holds it.</RecipeHeader.Description>
...
<UsageList usage={{"recipeName":"org.openrewrite.kubernetes.clusterapi.MigrateMachinePoolToV1beta2","displayName":"Migrate `MachinePool` to `cluster.x-k8s.io/v1beta2`","groupId":"org.openrewrite.recipe","artifactId":"rewrite-kubernetes","versionKey":"VERSION_ORG_OPENREWRITE_RECIPE_REWRITE_KUBERNETES","requiresConfiguration":false}}>
