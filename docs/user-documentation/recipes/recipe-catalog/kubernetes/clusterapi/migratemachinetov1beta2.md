---
title: "Migrate `Machine` to `cluster.x-k8s.io/v1beta2`"
sidebar_label: "Migrate `Machine` to `cluster.x-k8s.io/v1beta2`"
hide_title: true
---
...
<RecipeMeta
  displayName={"Migrate `Machine` to `cluster.x-k8s.io/v1beta2`"}
  description={"Rewrite a `Machine` into its `v1beta2` form. References name an API group rather than an `apiVersion`, and each `metav1.Duration` becomes whole seconds under `deletion`."}
  fqName={"org.openrewrite.kubernetes.clusterapi.MigrateMachineToV1beta2"}
...
<RecipeHeader
  ...
  appLink={"https://app.moderne.io/recipes/org.openrewrite.kubernetes.clusterapi.MigrateMachineToV1beta2"}
  markdownUrl={"https://raw.githubusercontent.com/moderneinc/moderne-docs/refs/heads/main/docs/user-documentation/recipes/recipe-catalog/kubernetes/clusterapi/migratemachinetov1beta2.md"}
  moderneOnly
>

<RecipeHeader.Title>Migrate `Machine` to `cluster.x-k8s.io/v1beta2`</RecipeHeader.Title>

<RecipeHeader.Description>Rewrite a `Machine` into its `v1beta2` form. References name an API group rather than an `apiVersion`, and each `metav1.Duration` becomes whole seconds under `deletion`.</RecipeHeader.Description>
...
<UsageList usage={{"recipeName":"org.openrewrite.kubernetes.clusterapi.MigrateMachineToV1beta2","displayName":"Migrate `Machine` to `cluster.x-k8s.io/v1beta2`","groupId":"org.openrewrite.recipe","artifactId":"rewrite-kubernetes","versionKey":"VERSION_ORG_OPENREWRITE_RECIPE_REWRITE_KUBERNETES","requiresConfiguration":false}}>
