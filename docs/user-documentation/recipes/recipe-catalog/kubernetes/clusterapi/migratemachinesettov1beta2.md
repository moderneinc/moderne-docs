---
title: "Migrate `MachineSet` to `cluster.x-k8s.io/v1beta2`"
sidebar_label: "Migrate `MachineSet` to `cluster.x-k8s.io/v1beta2`"
hide_title: true
---
...
<RecipeMeta
  displayName={"Migrate `MachineSet` to `cluster.x-k8s.io/v1beta2`"}
  description={"Rewrite a `MachineSet` into its `v1beta2` form. `spec.minReadySeconds` moves onto the machines it creates, since `MachineSetSpec` no longer holds it."}
  fqName={"org.openrewrite.kubernetes.clusterapi.MigrateMachineSetToV1beta2"}
...
<RecipeHeader
  ...
  appLink={"https://app.moderne.io/recipes/org.openrewrite.kubernetes.clusterapi.MigrateMachineSetToV1beta2"}
  markdownUrl={"https://raw.githubusercontent.com/moderneinc/moderne-docs/refs/heads/main/docs/user-documentation/recipes/recipe-catalog/kubernetes/clusterapi/migratemachinesettov1beta2.md"}
  moderneOnly
>

<RecipeHeader.Title>Migrate `MachineSet` to `cluster.x-k8s.io/v1beta2`</RecipeHeader.Title>

<RecipeHeader.Description>Rewrite a `MachineSet` into its `v1beta2` form. `spec.minReadySeconds` moves onto the machines it creates, since `MachineSetSpec` no longer holds it.</RecipeHeader.Description>
...
<UsageList usage={{"recipeName":"org.openrewrite.kubernetes.clusterapi.MigrateMachineSetToV1beta2","displayName":"Migrate `MachineSet` to `cluster.x-k8s.io/v1beta2`","groupId":"org.openrewrite.recipe","artifactId":"rewrite-kubernetes","versionKey":"VERSION_ORG_OPENREWRITE_RECIPE_REWRITE_KUBERNETES","requiresConfiguration":false}}>
