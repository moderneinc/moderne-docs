...
_Hibernate 6.x removed `UUIDCharType`, `UUIDBinaryType` and `PostgresUUIDType`. Replace `@Type` annotations referencing these with `@JdbcTypeCode` and the corresponding `SqlTypes` constant._
...
<RunRecipe
  recipeName="io.moderne.hibernate.update60.MigrateRemovedUuidTypes"
...
<RecipeCallout link="https://app.moderne.io/recipes/io.moderne.hibernate.update60.MigrateRemovedUuidTypes" />
