...
_When using `@NativeQuery` or `@Query(nativeQuery = true)`, enum parameters are not automatically converted by JPA. This recipe finds native queries with raw enum bind parameters that need SpEL expressions like `:#{#status.name()}` or `:#{#status.ordinal()}` depending on how the enum is persisted._
...
<RunRecipe
  recipeName="io.moderne.hibernate.update70.FindNativeQueryRawEnumParameters"
...
<RecipeCallout link="https://app.moderne.io/recipes/io.moderne.hibernate.update70.FindNativeQueryRawEnumParameters" />
