...
<RecipeHeader.Title>Find `pathMatch` route properties that may need type narrowing</RecipeHeader.Title>

<RecipeHeader.Description>Finds `pathMatch` property assignments in route configurations. In Angular 14, the `pathMatch` type was narrowed from `string` to `'full' | 'prefix'`. Routes defined as plain objects without explicit `Route` or `Routes` typing may fail type checking.</RecipeHeader.Description>
