...
* [Add `ChangeDetectionStrategy.Eager` to components](./add-eager-change-detection.md)
...
* [Add `istanbul-lib-instrument` for Karma code coverage](./add-istanbul-instrumenter.md)
* [Add `static: false` to view queries](./add-static-false-to-view-queries.md)
* [Add `strictTemplates: false` to `tsconfig.json`](./add-strict-templates-false.md)
* [Add `trustProxyHeaders` to the Angular app engine](./add-trust-proxy-headers.md)
* [Add `withNoIncrementalHydration()` to `provideClientHydration()`](./add-no-incremental-hydration.md)
* [Add `withXhr()` to `provideHttpClient()`](./add-with-xhr.md)
...
* [Remove `withFetch()` from `provideHttpClient()`](./remove-with-fetch.md)
* [Remove zone.js polyfill from angular.json](./remove-zone-js-polyfill.md)
* [Rename `ChangeDetectionStrategy.Default` to `ChangeDetectionStrategy.Eager`](./rename-default-change-detection.md)
...
* [Replace `createNgModuleRef` with `createNgModule`](./replace-create-ng-module-ref.md)
...
* [Split a `model()` that has a matching explicit output](./migrate-model-with-output.md)
* [Suppress the safe navigation extended diagnostics](./suppress-safe-navigation-diagnostics.md)
