...
_Find a GitLab Template in use._

## Recipe source

[GitHub: FindTemplate.java](https://github.com/openrewrite/rewrite-gitlab/blob/main/src/main/java/org/openrewrite/gitlab/search/FindTemplate.java),
...
* [Find YAML entries](../../yaml/search/findkey)
  * key: `$.include[?(@.template =~ 'null(?:@.+)?')].template`
...
Now that `com.yourorg.FindTemplateExample` has been defined, activate it and take a dependency on `org.openrewrite.recipe:rewrite-gitlab:{{VERSION_ORG_OPENREWRITE_RECIPE_REWRITE_GITLAB}}` in your build file:
...
rewrite {
    activeRecipe("com.yourorg.FindTemplateExample")
    ...
    <plugins>
      <plugin>
        ...
        <configuration>
          ...
          <activeRecipes>
            <recipe>com.yourorg.FindTemplateExample</recipe>
          </activeRecipes>
        </configuration>
        <dependencies>
          <dependency>
...
```shell title="shell"
mod run . --recipe FindTemplate --recipe-option "template=Terraform/Base.gitlab-ci.yml"
```
...
<RecipeCallout link="https://app.moderne.io/recipes/org.openrewrite.gitlab.search.FindTemplate" />
