---
description: GitLab OpenRewrite recipes.
---

# GitLab

_Recipes to perform [GitLab](https://gitlab.com/) migration tasks._

## Categories

* [Search](/user-documentation/recipes/recipe-catalog/gitlab/search)

## Composite Recipes

_Recipes that include further recipes, often including the individual recipes below._

* [GitLab CI best practices](./bestpractices.md)

## Recipes

* [Add GitLab component](./addcomponent.md)
* [Add GitLab stages](./addstages.md)
* [Add GitLab template](./addtemplate.md)
* [Add artifacts expire_in](./addartifactsexpirein.md)
* [Add cache configuration](./addcache.md)
* [Add default keyword](./adddefaultkeyword.md)
* [Add interruptible](./addinterruptible.md)
* [Add job timeout](./addtimeout.md)
* [Add retry configuration](./addretry.md)
* [Add workflow rules](./addworkflowrules.md)
* [Change GitLab Component](./changecomponent.md)
* [Change GitLab template](./changetemplate.md)
* [Migrate `only`/`except` to `rules`](./migratetorules.md)
* [Remove GitLab template](./removetemplate.md)


