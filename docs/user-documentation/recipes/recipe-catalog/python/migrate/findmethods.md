...
_Find function and method calls by pattern. Covers standalone functions, class methods, static methods, and constructor calls._
...
| --- | --- | --- | --- |
| `String` | methodPattern | A method pattern used to find matching function and method calls. The pattern format is `<module-or-type> <function-name>(<arg-patterns>)`. The module or type part is optional; when omitted, any declaring type matches. `..` matches any number of arguments. For example, `dumps(..)` finds all calls to any function named `dumps`, `json dumps(..)` finds only calls to `json.dumps()`, and `os.path join(..)` finds calls to `os.path.join()`. | `dumps(..)` |
...
displayName: Find Python function and method usages example
recipeList:
  - org.openrewrite.python.migrate.FindMethods:
      methodPattern: dumps(..)
```
...
<RecipeCallout link="https://app.moderne.io/recipes/org.openrewrite.python.migrate.FindMethods" />
