...
_Replace `typing.Type` with the built-in `type` type. Available in Python 3.9+ (PEP 585)._
...
[GitHub: search?type=code&q=repo:+org.openrewrite.python.migrate.ReplaceTypingTypeWithBuiltin](https://github.com/search?type=code&q=repo:+org.openrewrite.python.migrate.ReplaceTypingTypeWithBuiltin),
[Issue Tracker](),
...
mod run . --recipe org.openrewrite.python.migrate.ReplaceTypingTypeWithBuiltin
```
