---
sidebar_label: "Replace `time.clock()` with `time.perf_counter()`"
---
...
**org.openrewrite.python.migrate.ReplaceTimeClock**

_Replace `time.clock()` with `time.perf_counter()`. time.clock() was deprecated in Python 3.3 and removed in 3.8._
...
mod run . --recipe org.openrewrite.python.migrate.ReplaceTimeClock
```
