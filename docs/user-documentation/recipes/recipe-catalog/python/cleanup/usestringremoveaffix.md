...
<RecipeHeader.Title>Replace string slicing with `removeprefix`/`removesuffix`</RecipeHeader.Title>

<RecipeHeader.Description>Replace `if text.startswith(s): text = text[N:]` with `text = text.removeprefix(s)` and the equivalent `endswith` pattern with `removesuffix` (Python 3.9+).</RecipeHeader.Description>

</RecipeHeader>

<UsageList usage={{"recipeName":"org.openrewrite.python.cleanup.UseStringRemoveAffix","displayName":"Replace string slicing with `removeprefix`/`removesuffix`","pipPackage":"openrewrite-static-analysis","companionJars":[{"groupId":"org.openrewrite","artifactId":"rewrite-python","versionKey":"VERSION_ORG_OPENREWRITE_REWRITE_PYTHON"}]}}>
