...
<RecipeHeader.Title>Fold same-literal `==`/`!=` comparisons to boolean constants</RecipeHeader.Title>

<RecipeHeader.Description>When both sides of `==` or `!=` are the same literal, replace the expression with `True` or `False` respectively.</RecipeHeader.Description>

</RecipeHeader>

<UsageList usage={{"recipeName":"org.openrewrite.python.cleanup.EqualityIdentity","displayName":"Fold same-literal `==`/`!=` comparisons to boolean constants","pipPackage":"openrewrite-static-analysis","companionJars":[{"groupId":"org.openrewrite","artifactId":"rewrite-python","versionKey":"VERSION_ORG_OPENREWRITE_REWRITE_PYTHON"}]}}>
