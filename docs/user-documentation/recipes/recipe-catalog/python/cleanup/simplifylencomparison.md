...
<RecipeHeader.Title>Replace `len()` emptiness check with truthiness</RecipeHeader.Title>

<RecipeHeader.Description>Rewrite ``len(seq) > 0`` / ``len(seq) != 0`` to ``seq`` and ``len(seq) == 0`` to ``not seq``, leveraging Python's built-in truthiness for collections.</RecipeHeader.Description>

</RecipeHeader>

<UsageList usage={{"recipeName":"org.openrewrite.python.cleanup.SimplifyLenComparison","displayName":"Replace `len()` emptiness check with truthiness","pipPackage":"openrewrite-static-analysis","companionJars":[{"groupId":"org.openrewrite","artifactId":"rewrite-python","versionKey":"VERSION_ORG_OPENREWRITE_REWRITE_PYTHON"}]}}>
