...
<RecipeHeader.Title>Drop redundant `.keys()` on dict iteration</RecipeHeader.Title>

<RecipeHeader.Description>Dictionaries iterate over their keys by default, making explicit `.keys()` calls unnecessary in for-loops and `in` expressions.</RecipeHeader.Description>

</RecipeHeader>

<UsageList usage={{"recipeName":"org.openrewrite.python.cleanup.RemoveDictKeys","displayName":"Drop redundant `.keys()` on dict iteration","pipPackage":"openrewrite-static-analysis","companionJars":[{"groupId":"org.openrewrite","artifactId":"rewrite-python","versionKey":"VERSION_ORG_OPENREWRITE_REWRITE_PYTHON"}]}}>
