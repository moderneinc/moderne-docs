...
<RecipeHeader.Title>Narrow bare `except:` to `except Exception:`</RecipeHeader.Title>

<RecipeHeader.Description>An unqualified `except:` intercepts every exception, including `SystemExit` and `KeyboardInterrupt`. Specifying `Exception` restricts the handler to ordinary runtime errors.</RecipeHeader.Description>

</RecipeHeader>

<UsageList usage={{"recipeName":"org.openrewrite.python.cleanup.DoNotUseBareExcept","displayName":"Narrow bare `except:` to `except Exception:`","pipPackage":"openrewrite-static-analysis","companionJars":[{"groupId":"org.openrewrite","artifactId":"rewrite-python","versionKey":"VERSION_ORG_OPENREWRITE_REWRITE_PYTHON"}]}}>
