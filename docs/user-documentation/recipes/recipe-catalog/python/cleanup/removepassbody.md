...
<UsageList usage={{"recipeName":"org.openrewrite.python.cleanup.RemovePassBody","displayName":"Drop ``pass``-only ``if`` body by inverting the guard","pipPackage":"openrewrite-static-analysis","companionJars":[{"groupId":"org.openrewrite","artifactId":"rewrite-python","versionKey":"VERSION_ORG_OPENREWRITE_REWRITE_PYTHON"}]}}>
