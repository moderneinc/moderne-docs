...
<UsageList usage={{"recipeName":"org.openrewrite.python.cleanup.RemoveRedundantPathExists","displayName":"Drop ``exists()`` check before ``is_dir()``/``is_file()``","pipPackage":"openrewrite-static-analysis","companionJars":[{"groupId":"org.openrewrite","artifactId":"rewrite-python","versionKey":"VERSION_ORG_OPENREWRITE_REWRITE_PYTHON"}]}}>
