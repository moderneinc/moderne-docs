...
[GitHub: AppendToTextFile.java](https://github.com/openrewrite/rewrite/blob/main/rewrite-core/src/main/java/org/openrewrite/text/AppendToTextFile.java),
...
| --- | --- | --- | --- |
| `String` | relativeFileName | File name, using a relative path. May also be a glob expression (e.g. `**/*.txt`) to match one or more existing files; when a glob is supplied no new file is created. If a non-plaintext file matches, that file is left unchanged. | `foo/bar/baz.txt` |
...
<RunRecipe
  recipeName="org.openrewrite.text.AppendToTextFile"
  ...
  cliOptions={' --recipe-option "relativeFileName=foo/bar/baz.txt" --recipe-option "content=Some text." --recipe-option "preamble=# File generated by OpenRewrite #"'}
  showGradle={false}
  showMaven={false}
  hasDataTables
/>
