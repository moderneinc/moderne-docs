---
description: JSON OpenRewrite recipes.
---

# JSON

_Basic building blocks for transforming JSON._

## Categories

* [Format](/user-documentation/recipes/recipe-catalog/json/format)
* [Search](/user-documentation/recipes/recipe-catalog/json/search)

## Recipes

* [Add value to JSON Object](./addkeyvalue.md)
* [Change key](./changekey.md)
* [Change value](./changevalue.md)
* [Copy JSON value](./copyvalue.md)
* [Create JSON file](./createjsonfile.md)
* [Delete key](./deletekey.md)
* [Remove empty keys](./removeemptykeys.md)


