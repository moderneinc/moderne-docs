...
<RecipeMeta
  ...
  fqName={"org.openrewrite.docker.search.FindRootUser"}
  languages={["OpenRewrite"]}
  license={"Apache License Version 2.0"}
  sourceUrl={"https://github.com/openrewrite/rewrite/blob/main/rewrite-docker/src/main/java/org/openrewrite/docker/search/FindRootUser.java"}
/>
...
<ExampleList examples={[{"parameters":[{"parameter":"includeMissingUser","value":"null"}],"variants":[{"language":"docker","before":"FROM ubuntu:22.04\nRUN apt-get update\n","after":"~~(No USER instruction, runs as root)~~>FROM ubuntu:22.04\nRUN apt-get update\n","diff":"@@ -1,1 +1,1 @@\n-FROM ubuntu:22.04\n+~~(No USER instruction, runs as root)~~>FROM ubuntu:22.04\nRUN apt-get update\n","newFile":false}]}]}>
