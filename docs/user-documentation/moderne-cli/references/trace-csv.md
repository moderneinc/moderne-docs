---
sidebar_label: trace.csv fields
...
| `mcpToolName` | string | Name of the MCP tool invoked | `find_recipes` |
...
| `mcpArguments` | string | JSON arguments passed to the tool | `{"query":"order imports"}` |
| `mcpElapsedTimeMs` | integer | Duration in milliseconds | `540` |

## organization

The final column in every row.
