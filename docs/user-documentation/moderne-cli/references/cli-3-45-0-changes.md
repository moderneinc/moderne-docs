...
Telemetry has evolved considerably in CLI 4.x, which ships a supported `modw` wrapper and uploads telemetry to your tenant automatically at the end of each command, or on demand with `mod telemetry publish`. For current, complete documentation, see:
