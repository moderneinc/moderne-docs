...
| gradleVersion     | `false`  | Selects a specific Gradle installation to use when building LSTs for repositories that do not have a Gradle wrapper. The version must match one of the Gradle installations known to the CLI (detected from `$PATH`, `$GRADLE_HOME`, SDKMAN, Homebrew, or manually configured via `mod config build gradle installation edit`). During `mod git sync csv`, this value is written to each repository's `.moderne` directory. Use `mod config build gradle installation list` to see available versions.<br /><br />Example: `8.1.1` or `4.5` |
