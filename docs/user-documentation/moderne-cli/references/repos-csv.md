...
2. **Moderne Platform org hierarchy** - Where you provide this file to the Moderne Connector that is then used to create organizations in the platform
...
| branch            | `false`  | The branch of the repository that should be checked out. If empty, the CLI uses the remote's default branch. Required when `changeset` is specified.<br /><br />Example: `main`                                                                                                                                                                                                                                                              |
...
| changeset         | `false`  | If provided, this will check out the repository at this specific commit SHA. Requires a `branch` value on the same row. <br /><br />Example: `aa5f25ac0031`                                                                                                                                                                                                                                  |
| gradleArgs        | `false`  | Build arguments that are added to the end of the Gradle command line when building LSTs. During `mod git sync csv`, these values are written to each repository's `.moderne` directory and will overwrite any existing values. See [configuration precedence](../how-to-guides/layer-config-cli.md#configuration-precedence) for details.<br /><br />Example: `-Dmyprop=myvalue`                                                                                                                                                                                                                   |
| gradleVersion     | `false`  | Selects a specific Gradle version to use when building LSTs. Takes precedence over the repository's Gradle wrapper; leave it empty to use the wrapper. The version must exactly match one of the Gradle installations known to the CLI (detected from `$PATH`, `$GRADLE_HOME`, SDKMAN, Homebrew, or manually configured via `mod config build gradle installation edit`). During `mod git sync csv`, this value is written to each repository's `.moderne` directory. Use `mod config build gradle installation list` to see available versions.<br /><br />Example: `8.1.1` |
...
| nodeOptions       | `false`  | Node.js options set via the `NODE_OPTIONS` environment variable when building LSTs and running recipes. During `mod git sync csv`, these values are written to each repository's `.moderne` directory and will overwrite any existing values. See [configuration precedence](../how-to-guides/layer-config-cli.md#configuration-precedence) for details.<br /><br />Example: `--max-old-space-size=8192`                                                                                                                                                                                                                                   |
| nodeVersion       | `false`  | Configures the Node.js version to use during LST generation. During `mod git sync csv`, this value is written to each repository's `.moderne` directory.<br /><br />Example: `18` or `20.11.1`                                                                                                                                                                                                                   |
| mavenArgs         | `false`  | Build arguments that are added to the end of the Maven command line when building LSTs. During `mod git sync csv`, these values are written to each repository's `.moderne` directory and will overwrite any existing values. See [configuration precedence](../how-to-guides/layer-config-cli.md#configuration-precedence) for details.<br /><br />Example: `-Pfast`                                                                                                                                                                                                                                   |
| mavenVersion      | `false`  | Selects a specific Maven version to use when building LSTs. Takes precedence over the repository's Maven wrapper; leave it empty to use the wrapper. The version must exactly match one of the Maven installations known to the CLI (detected from `$PATH`, `$MAVEN_HOME`, `$MVN_HOME`, SDKMAN, Homebrew, or manually configured via `mod config build maven installation edit`). During `mod git sync csv`, this value is written to each repository's `.moderne` directory. Use `mod config build maven installation list` to see available versions.<br /><br />Example: `3.9.6` |
| org*              | `false`  | Defines the [organizational hierarchy](#defining-an-organizational-hierarchy). Each column specifies an organization the repository should be part of. The column name should be `org` plus a number such as: `org1,org2,org3`. There is no limit for how many orgs you can define. These columns are optional for local CLI use, but **at least one org column is required when providing this file to the Moderne Connector**.<br /><br />Example: `openrewrite` |
...
The Moderne Platform uses a combination of `origin`, `path`, and `branch` to uniquely identify a specific repository. In general, all repositories coming from the same SCM server should have the same `origin` – whereas the `path` and `branch` will differ. The combination of `origin`, `path`, and `branch` must uniquely identify each row in your `repos.csv`. For that reason, we recommend setting `branch` explicitly when you provide this file to the Moderne Connector, even though the CLI can resolve it from the remote.
...
Repo fetchers intentionally do not assign organizations, as how repositories map to organizations is a decision that belongs to you rather than your source code manager. After generating the base repository list, define your own hierarchy by adding org columns as described below.

## Defining an organizational hierarchy

An organizational hierarchy doesn't need to be complete from day one. A common way to start is with a single `ALL` organization that contains every repository, plus one organization for a single team. You can then expand to more organizations and more levels of hierarchy over time.
