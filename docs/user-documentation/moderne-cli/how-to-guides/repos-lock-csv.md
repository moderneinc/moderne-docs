...
A published `repos-lock.csv` is also how you share and download pre-built LSTs when you use only the Moderne CLI (i.e., with no Moderne Platform or Moderne DX tenant). One machine builds and publishes the LSTs to an artifact repository (such as Artifactory, S3, Google Cloud Storage, or Azure Blob Storage) and records their locations in the file. Everyone else then downloads the LSTs straight from the artifact repository.
...
Object storage is also supported – see `mod config lsts artifacts s3 edit`, `mod config lsts artifacts gcs edit`, and `mod config lsts artifacts azure-blob edit`. For example, to publish to a Google Cloud Storage bucket:
...
The `repos-lock.csv` argument accepts a local path or a remote URI (HTTP(S), `s3://`, `gs://`, or Azure Blob), so you can point directly at the central file published in the previous step.
