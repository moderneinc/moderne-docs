...
:::tip
If you use only the Moderne CLI – with no Moderne Platform or DX tenant – a published `repos-lock.csv` is how you share and download pre-built LSTs through an artifact repository. See [Downloading LSTs without a Moderne tenant](./cli-only-lst-distribution.md) for that end-to-end workflow.
:::
