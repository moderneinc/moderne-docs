...
A published `repos-lock.csv` is also how you share and download pre-built LSTs when you use only the Moderne CLI – with no Moderne Platform or Moderne DX (Developer Experience) tenant. One machine builds and publishes the LSTs to an artifact repository (such as Artifactory, S3, or Azure Blob Storage), records their locations in the file, and everyone else downloads them straight from there. No Moderne tenant is required.
...
So that downloads can authenticate, team members first configure the same artifact repository on their machine:
