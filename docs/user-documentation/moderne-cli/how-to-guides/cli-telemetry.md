...
The build step is optional. `mod git sync` can download prebuilt LSTs rather than source, and `mod run` then follows sync directly. That is what happens when a central team runs [mass ingest](../../../administrator-documentation/moderne-platform/how-to-guides/mass-ingest.md) and everyone else runs recipes against the published LSTs, as in [mass run](../../../administrator-documentation/moderne-dx/how-to-guides/mass-run-dx.md).

Those traces still carry build columns. Each LST records the build that produced it, and a downloaded LST contributes that record to the trace, so populated build columns don't necessarily mean a build ran locally. See [build columns and prebuilt LSTs](../references/trace-csv.md#build-columns-and-prebuilt-lsts).
...
:::note
MCP telemetry is always-on and CSV-only: each tool call is appended straight to the `~/.moderne/cli/trace/mcp/` queue and is never written to a `trace.json`. For a CLI signed in to a tenant, these rows [upload the same way](#how-the-cli-uploads-telemetry-to-your-tenant) as other command telemetry.
:::
...
| Field               | Type   | Description                                                                                                                                                                                                        |
|---------------------|--------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `outcome`           | string | Session outcome (e.g., `Succeeded`, `Failed`)                                                                                                                                                                      |
| `startTime`         | string | ISO 8601 timestamp when the agent started                                                                                                                                                                          |
| `endTime`           | string | ISO 8601 timestamp when the agent exited                                                                                                                                                                           |
| `id`                | string | Unique identifier for this `mod <agent> chat` run                                                                                                                                                                  |
| `syncId`            | string | Identifier of the sync that produced the organization state the session ran on, either the sync this command ran or the most recent earlier sync of the directory. It matches the command ID of that sync's trace. |
| `name`              | string | The agent command that was run (e.g., `claude`, `codex`, `copilot`)                                                                                                                                                |
| `version`           | string | The agent's installed version, when it can be determined                                                                                                                                                           |
| `prompt`            | string | The session's initial prompt, from `--prompt` or read from the agent's transcript                                                                                                                                  |
| `sessionId`         | string | The agent's own identifier for the session                                                                                                                                                                         |
| `turns`             | number | Messages the user sent, including the initial prompt                                                                                                                                                               |
| `inputTokens`       | number | Input tokens for the session, including cached tokens                                                                                                                                                              |
| `cachedInputTokens` | number | The part of `inputTokens` served from the agent's cache                                                                                                                                                            |
| `outputTokens`      | number | Output tokens for the session                                                                                                                                                                                      |
| `cost`              | number | Session cost, as the agent itself accounts for it                                                                                                                                                                  |
...
| CLI signed in to a tenant (happy path)             | `cli` / `mcp` | Queued locally, then uploaded to Moderne's bucket through the tenant gateway at the end of each command (or with `mod telemetry publish`) | No                             | Yes                                                                                                                                                            | Replication alongside the `saas` data; a wrapper can *also* publish to a destination you control                                                 |
| CLI not signed in, but your org is a SaaS customer | `cli` / `mcp` | Queued locally; nothing reaches the gateway while disconnected                                                                            | No                             | Not while disconnected. Once you're signed in, the queue uploads at the end of the next command (but a short-lived host can spin down before it ever signs in) | Replication once the queue flushes; use a wrapper to your own destination if the host may never sign in (for example, ephemeral mass-ingest VMs) |
...
With this on, each command waits up to 30 seconds for the queue to upload, and prints a line to stderr if anything is left undelivered. The setting requires CLI 4.8.1 or later and is off by default. To turn it off again, run `mod config telemetry wait-for-upload --enabled=false`.

To upload the whole queue on demand with no time limit (for example, right before pulling a report), run `mod telemetry publish`. It reports the result for each file and exits with an error if the gateway permanently rejects any of them.

The queue lives in `~/.moderne/cli/trace/`, with one directory per command type. As files upload, the CLI sorts them into subdirectories:

| Location           | Contents                                                                                                                 |
|--------------------|--------------------------------------------------------------------------------------------------------------------------|
| `<type>/`          | Telemetry waiting to upload                                                                                              |
| `<type>/pushed/`   | Telemetry that uploaded successfully                                                                                     |
| `<type>/failed/`   | Telemetry the gateway refused, retried on later uploads                                                                  |
| `<type>/rejected/` | Telemetry the gateway rejected as invalid, never retried. A `.error` file next to each one holds the gateway's response. |
| `telemetry.log`    | Diagnostic log of upload attempts                                                                                        |
