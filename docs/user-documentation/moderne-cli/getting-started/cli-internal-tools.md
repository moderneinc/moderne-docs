...
* **Recipe modules** — see [the latest versions of every OpenRewrite module](../../recipes/lists/latest-versions-of-every-openrewrite-module.md) reference for the full list of recipe JARs and a ready-to-run CLI install command. The reference includes both the open-source OpenRewrite recipes (`org.openrewrite.*`) and Moderne's proprietary recipes (`io.moderne.recipe`).
...
curl -fL -o moderne-cli.sh \
    "https://internal-mirror.example.com/io/moderne/moderne-cli-linux/4.2.1/moderne-cli-linux-uln-suppress-simpleclient-tracer-otel-fp.sh"
```
...
```powershell
Invoke-WebRequest `
    -Uri "https://internal-mirror.example.com/io/moderne/moderne-cli-windows/4.2.1/moderne-cli-windows-uln-suppress-simpleclient-tracer-otel-fp.zip" `
