...
Moddy Desktop is a standalone version of Moddy that runs on your local machine. This allows you to quickly run it against your local repositories or connect to other tools such as Claude code through an MCP connection. We also offer a web-based [Moddy](./moddy-platform.md) – which runs inside the Moderne Platform so that you can run requests against thousands of repositories at once.
