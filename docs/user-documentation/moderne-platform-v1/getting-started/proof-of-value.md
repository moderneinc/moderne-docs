...
<VersionBanner version="v1" linkPath="/user-documentation/moderne-platform/getting-started/proof-of-value" />
...
2. **Mass Ingest** - Your team [sets up an ingestion pipeline](../../../administrator-documentation/moderne-platform-v1/how-to-guides/mass-ingest.md) to build and publish LST artifacts for your repositories. You should start with 100 or more diverse repositories for best results. This step does not require you to make any changes to the repositories themselves (such as installing build plugins).

3. **Agent setup** - Your team sets up the Moderne agent following our [on-premise agent configuration doc](../../../administrator-documentation/moderne-platform-v1/how-to-guides/agent-configuration/agent-config.md). The agent runs as a Docker image or JAR and connects to your source code manager (SCM) and artifact repository using read-only service accounts (takes less than 1 hour with accounts ready).
...
1. **Download the Moderne CLI** – Download the [latest JAR from Maven Central](https://central.sonatype.com/artifact/io.moderne/moderne-cli/versions).
    * While not required, you are strongly encouraged to set up an alias for running the JAR.
...
### [Spring Boot 3.5 best practices](../../recipes/recipe-catalog/java/spring/boot3/springboot3bestpractices.md)
