...
If the CLI is installed on the system PATH (e.g., with Homebrew or Chocalatey), you'll want to select the top radio button, and the plugin should automatically find the CLI. If you have downloaded the CLI as a JAR (for example, to an internal artifact repository), you'll want to pick the second radio button and specify the path where it has been downloaded on disk:
