...
Once data is landing in your bucket or container, you can query it directly with any engine that reads CSV from object storage. If you haven't configured replication yet, see the [AWS](./aws-replication.md) or [Azure](./azure-replication.md) guides first. The Hive partition layout means every major engine can prune to just the partitions a query needs.
...
**Recipe-run summary for a specific day** — what recipes ran, how many repos they touched, and estimated time saved:
...
**Top recipes producing committed code** — recipes whose runs actually shipped (the value story for impact reviews):
