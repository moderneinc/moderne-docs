...
* **(Recommended) Include publish URIs in the CSV.** When each row has a `publishUri` column, the Connector trusts those values and fetches LSTs directly. You can generate such a CSV by setting up a [Mass Ingest](../mass-ingest.md) pipeline; its `mod publish` step maintains a [central `repos-lock.csv`](../../../../user-documentation/moderne-cli/how-to-guides/repos-lock-csv.md#the-central-repos-lockcsv-file) at the root of the artifact repository with `publishUri` values for every repository. If you also host a `repos.csv` at the root of the artifact repository, `mod publish` uses its organizational hierarchy when rebuilding the central `repos-lock.csv` on every publish.
