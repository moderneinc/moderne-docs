...
| `MODERNE_ORGANIZATION_SOURCES_HTTP_{index}_ENCRYPT`                  | `false`  | `true`  | Whether the Connector fetches each LST from this source, encrypts it with your symmetric key, and uploads it to Moderne. See [Encrypting LSTs](#encrypting-lsts) below for the rules that apply per source type. |
| `MODERNE_ORGANIZATION_SOURCES_FILE_{index}_ENCRYPT`                  | `false`  | `true`  | Whether the Connector fetches each LST from this source, encrypts it with your symmetric key, and uploads it to Moderne. See [Encrypting LSTs](#encrypting-lsts) below for the rules that apply per source type. |
...
| `--moderne.organization.sources.http[{index}].encrypt`                    | `false`  | `true`  | Whether the Connector fetches each LST from this source, encrypts it with your symmetric key, and uploads it to Moderne. See [Encrypting LSTs](#encrypting-lsts) below for the rules that apply per source type. |
| `--moderne.organization.sources.file[{index}].encrypt`                    | `false`  | `true`  | Whether the Connector fetches each LST from this source, encrypts it with your symmetric key, and uploads it to Moderne. See [Encrypting LSTs](#encrypting-lsts) below for the rules that apply per source type. |
...
| `MODERNE_ORGANIZATION_SOURCES_S3_{index}_ENCRYPT`                  | `false`                                                  | `true`  | Whether the Connector fetches each LST from this source, encrypts it with your symmetric key, and uploads it to Moderne. See [Encrypting LSTs](#encrypting-lsts) below for the rules that apply per source type. |
...
| `--moderne.organization.sources.s3[{index}].encrypt`                    | `false`                                                  | `true`  | Whether the Connector fetches each LST from this source, encrypts it with your symmetric key, and uploads it to Moderne. See [Encrypting LSTs](#encrypting-lsts) below for the rules that apply per source type. |
...
| `MODERNE_ORGANIZATION_SOURCES_GCS_{index}_ENCRYPT`         | `false`                                                         | `true`  | Whether the Connector fetches each LST from this source, encrypts it with your symmetric key, and uploads it to Moderne. See [Encrypting LSTs](#encrypting-lsts) below for the rules that apply per source type. |
...
| `--moderne.organization.sources.gcs[{index}].encrypt`          | `false`                                                         | `true`  | Whether the Connector fetches each LST from this source, encrypts it with your symmetric key, and uploads it to Moderne. See [Encrypting LSTs](#encrypting-lsts) below for the rules that apply per source type. |
...
|-------------|------------------|--------------------------------------------------------------------------------------------------------------------|
| `http`      | Supported        | Moderne reads the endpoint through the tunnel.                                                                     |
| `s3`        | Supported        | Moderne reads the bucket through the tunnel.                                                                       |
| `gcs`       | Not supported    | Cloud Storage sources cannot be tunneled. The Connector fails to start on a pass-through Cloud Storage source.     |
| `file`      | Not useful       | A local file cannot be read by Moderne. Use a `poll` block so the Connector discovers and uploads the LSTs itself. |

A `file` source with the default `encrypt: true` also requires a `poll` block, because a local CSV carries no credentials the Connector could use to fetch the artifacts it names. The Connector fails to start otherwise.

## Recipe worker Maven variables

These configure the `settings.xml` that recipes use when resolving dependencies during a run, usually a `<mirrors>` entry so runs resolve through an internal repository rather than Maven Central.
...
| Variable Name                              | Required | Default | Description                                                                                                                                                                                                                                                                                                           |
|--------------------------------------------|----------|---------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `MODERNE_RECIPE_WORKER_MAVEN_SETTINGSXML`  | `false`  |         | An inline `settings.xml` document. Takes precedence over `settings-path`. It must not contain a `servers` element: this configuration reaches Moderne, so credentials in it would leave your network. Register the repository under `moderne.connector.http-tool` instead, and the Connector injects its credentials. |
| `MODERNE_RECIPE_WORKER_MAVEN_SETTINGSPATH` | `false`  |         | Path to a `settings.xml` already present on the recipe worker. Paths on the Connector host do not exist there.                                                                                                                                                                                                        |
</TabItem>
...
| Argument Name                                 | Required | Default | Description                                                                                                                                                                                                                                                                                                           |
|-----------------------------------------------|----------|---------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `--moderne.recipe.worker.maven.settings-xml`  | `false`  |         | An inline `settings.xml` document. Takes precedence over `settings-path`. It must not contain a `servers` element: this configuration reaches Moderne, so credentials in it would leave your network. Register the repository under `moderne.connector.http-tool` instead, and the Connector injects its credentials. |
| `--moderne.recipe.worker.maven.settings-path` | `false`  |         | Path to a `settings.xml` already present on the recipe worker. Paths on the Connector host do not exist there.                                                                                                                                                                                                        |
</TabItem>
