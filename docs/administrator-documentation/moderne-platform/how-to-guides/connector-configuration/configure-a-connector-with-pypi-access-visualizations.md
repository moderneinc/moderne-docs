...
| `MODERNE_RECIPE_MARKETPLACE_REPOSITORIES_PYPI_{index}_BEARERTOKEN`              | `false`  |         | The bearer token (access token) used to access the index. <br/><br/>**Note:** Only one of basic auth (username+password) or bearer token can be used. If `bearer-token` is specified, username and password must not be provided. |
...
This is not required if you do not wish to install visualizations from your own PyPI package index.
